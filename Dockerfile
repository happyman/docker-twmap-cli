...
## 04c7861 with agent option
    git clone https://github.com/happyman/twmap && cd twmap && git checkout 135075e && \
    cd twmap_gen && composer install && cp config.inc.php.sample config.inc.php && \
    mkdir -p /workdir && chmod -R a+rwX /workdir && \
    curl  --connect-timeout 2 --max-time 30 --retry 99 -o /tmp/sorted_array_web http://make.happyman.idv.tw/~happyman/stb/sorted_array_web && \
    curl  --connect-timeout 2 --max-time 30 --retry 99  -o /tmp/sorted_array_web-97 http://make.happyman.idv.tw/~happyman/stb/sorted_array_web-97
